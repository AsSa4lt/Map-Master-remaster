//
//  RatingGameCapitalOfCountry.swift
//  Map Master
//
...
struct RatingGameCapitalOfCountry: View {
                ...
                if before_game > 0{
                    ...
                    Text("Time Left: \(String(format: "%.0f", ceil(timeLeft*100)/100)) s.").font(.largeTitle).fontWeight(.heavy).foregroundColor(.white)
                        VStack{
                            ...
                            Text("\(task[correct_answer-1])") .font(.largeTitle).fontWeight(.heavy).foregroundColor(Color.white)
                            Image("\(task[correct_answer-1])").resizable().frame(width: UIScreen.main.bounds.width*0.5, height: UIScreen.main.bounds.width*0.33).cornerRadius(10).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.orange, lineWidth: 4)).shadow(radius: 2).padding(.bottom)
                ...
                if before_game <= 0{
                    if timeLeft > 0{
                        ...
                        if score > GlobalUserData.rating_country_capital{
                            ...
                            GlobalUserData.set_rating_country_capital()
                        }
                    }
                }
            }
...
struct RatingGameCapitalOfCountry_Previews: PreviewProvider {
    static var previews: some View {
        RatingGameCapitalOfCountry(GlobalUserData: userData())
    }
}
