//
//  RatingGameFlagFromCountry.swift
//  Map Master
//
...
struct RatingGameFlagFromCountry: View {
                ...
                if before_game > 0{
                    ...
                    Text("Time Left: \(String(format: "%.0f", ceil(timeLeft*100)/100)) s.").font(.largeTitle).fontWeight(.heavy).foregroundColor(.white)
                        VStack{
                            ...
                            Image("\(task[correct_answer-1])").resizable() .frame(width: UIScreen.main.bounds.width*0.8, height: UIScreen.main.bounds.width*0.45).cornerRadius(10).overlay(RoundedRectangle(cornerRadius: 10)
                ...
                if before_game <= 0{
                    if timeLeft > 0{
                        ...
                        if score > GlobalUserData.rating_country_flag{
                            ...
                            GlobalUserData.set_rating_country_flag()
                        }
...
struct RatingGameFlagFromCountry_Previews: PreviewProvider {
    static var previews: some View {
        RatingGameFlagFromCountry(GlobalUserData: userData())
    }
}
