...
struct UnitedStatesStart: View {
    ...
    var body: some View {
        ZStack{
            MainBack()
            VStack{
                Text("Find US States!").fontWeight(.heavy).foregroundColor(.white).font(.system(size: 35))
                InteractiveMap(svgName: "usa-low") { pathData in // or just use $0
                ...
                }.padding(.vertical, -30.0).pickerStyle(.wheel)
                Spacer()
                NavigationLink(destination: UnitedStatesGame(GlobalUserData: GlobalUserData, choice: choice)) {
...
struct UnitedStatesStart_Previews: PreviewProvider {
    static var previews: some View {
        UnitedStatesStart(GlobalUserData: userData())
    }
}
