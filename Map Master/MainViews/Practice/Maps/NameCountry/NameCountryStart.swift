...
struct NameCountryStart: View {
    ...
    var body: some View {
        ZStack{
            MainBack()
            VStack{
                Text("Name all Countries!").fontWeight(.heavy).foregroundColor(.white).font(.system(size: 35))
                ...
                Spacer()
                NavigationLink(destination: NameCountriesGamePractice(GlobalUserData: GlobalUserData, choice: choice)) {
...
struct NameCountryStart_Previews: PreviewProvider {
    static var previews: some View {
        NameCountryStart(GlobalUserData: userData())
    }
}
