...
struct GuessCountryFromBorder: View {
    ...
    var body: some View {
        ZStack{
            MainBack()
            VStack{
                Text("Guess that country!").fontWeight(.heavy).foregroundColor(.white).font(.system(size: 35))
                Image("CyprusB").resizable().frame(width: UIScreen.main.bounds.width*0.6, height: UIScreen.main.bounds.width*0.6).cornerRadius(10).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.orange, lineWidth: 4)).shadow(radius: 2).padding(.vertical)
                
                
                Text("Highest score: \(GlobalUserData.result_country_border[choice-1])").fontWeight(.heavy).foregroundColor(.white).font(.system(size: 30))
                ...
                Spacer()
                NavigationLink(destination: GuessCountryFromBorderGame(GlobalUserData: GlobalUserData, choice: choice)) {
...
struct GuessCountryFromBorder_Previews: PreviewProvider {
    static var previews: some View {
        GuessCountryFromBorder(GlobalUserData: userData())
    }
}
