            ...
            VStack{
                if current < 10{
                    VStack{
                        ...
                        Text("\(unit2level9[current][0])") .font(.largeTitle).fontWeight(.heavy).foregroundColor(Color.white).padding(.top)
                        Image("\(unit2level9[current][0])").resizable().frame(width: UIScreen.main.bounds.width*0.6, height: UIScreen.main.bounds.width*0.4).cornerRadius(10).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.orange, lineWidth: 4)).shadow(radius: 2).padding(.bottom)
                        
                        ForEach(1..<unit2level9[0].count, id: \.self){ i in
                            Button{
                                if isTapped == false{
                                    isTapped = true
                                    show_answers = true
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                                        if unit2answers9[current] == i-1{
                                            correct = correct + 0.1
                                        }else{
                                    ...
                                    if correct >= 0.7{
                                        GlobalUserData.unit2[8] = true
                                    }
                                }
                            }label: {
                                if show_answers == false{
                                    Color.cyan.overlay(
                                        Text("\(unit2level9[current][i])").font(.title2).fontWeight(.heavy).foregroundColor(.white)
...
struct Unit2Lesson9_Previews: PreviewProvider {
    static var previews: some View {
        Unit2Lesson9(GlobalUserData: userData())
    }
}
