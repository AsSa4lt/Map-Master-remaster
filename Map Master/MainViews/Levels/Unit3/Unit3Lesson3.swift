            ...
            VStack{
                if current < 10{
                    VStack{
                        ...
                        ForEach(0..<level3unit3[0].count, id: \.self){ i in
                            Button{
                                if isTapped == false{
                                    isTapped = true
                                    answer = i
                                    ...
                                    if correct >= 0.7{
                                        GlobalUserData.unit3[2] = true
                                    }
                                }
                            }label: {
                                if show_answers == false{
                                    Color.indigo.overlay(
                                        Text("\(level3unit3[current][i])").font(.title2).fontWeight(.heavy).foregroundColor(.white)
...
struct Unit3Lesson3_Previews: PreviewProvider {
    static var previews: some View {
        Unit3Lesson3(GlobalUserData: userData())
    }
}
