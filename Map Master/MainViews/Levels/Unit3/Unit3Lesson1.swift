            ...
            VStack{
                if current < 10{
                    VStack{
                        ...
                        Image("\(level1unit3[current][answers1unit3[current]])").resizable().frame(width: UIScreen.main.bounds.width*0.6, height: UIScreen.main.bounds.width*0.4).cornerRadius(10).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.orange, lineWidth: 4)).shadow(radius: 2).padding(.bottom, 50.0).padding(.top)
                        
                        
                        ForEach(0..<level1unit3[0].count, id: \.self){ i in
                            Button{
                                if isTapped == false{
                                    isTapped = true
                                    answer = i
                                ...
                                if show_answers == false{
                                    Color.indigo.overlay(
                                        Text("\(level1unit3[current][i])").font(.title2).fontWeight(.heavy).foregroundColor(.white)
...
struct Unit3Lesson1_Previews: PreviewProvider {
    static var previews: some View {
        Unit3Lesson1(GlobalUserData: userData())
    }
}
