            ...
            VStack{
                if current < 10{
                    VStack{
                        ...
                        Text("\(level5unit3[current][answers5unit3[current]])") .font(.largeTitle).fontWeight(.heavy).foregroundColor(Color.white).padding(.top)
                        
                        
                        ForEach(0..<level5unit3[0].count, id: \.self){ i in
                            Button{
                                if isTapped == false{
                                    isTapped = true
                                    answer = i
                                    ...
                                    if correct >= 0.7{
                                        GlobalUserData.unit3[4] = true
                                        GlobalUserData.unit3Change = true
                                    }
                                }
                            }label: {
                                if show_answers == false{
                                    Image("\(level5unit3[current][i])B").resizable().frame(width: UIScreen.main.bounds.width*0.5, height: UIScreen.main.bounds.width*0.5).cornerRadius(10).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.orange, lineWidth: 4)).shadow(radius: 2).padding(.bottom)
                                }else{
                                    if answers5unit3[current] == i{
                                        Color.green.overlay(
                                            Text("\(level5unit3[current][i])").font(.title2).fontWeight(.heavy).foregroundColor(.white)
...
struct Unit3Lesson5_Previews: PreviewProvider {
    static var previews: some View {
        Unit3Lesson5(GlobalUserData: userData())
    }
}
