            ...
            VStack{
                if current < 10{
                    VStack{
                        ...
                        Image("\(level[current][answers[current]])").resizable().frame(width: UIScreen.main.bounds.width*0.6, height: UIScreen.main.bounds.width*0.4).cornerRadius(10).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.orange, lineWidth: 4)).shadow(radius: 2).padding(.bottom, 50.0).padding(.top)
                        
                        ThreeFlagAnswerButton(isTapped: $isTapped, GlobalUserData: GlobalUserData, current: $current, answer: $answer, correct: $correct, show_answers: $show_answers, answers: answers, level: level, what_to_change: $unit_bool_change, unit_change: $unit_change)
