...
import SwiftUI
public let unit4lesson10: [[String]] = [
    ["US-AK", "Alaska"],//1
    ["US-CA", "California"],//2
    ["US-TX", "Texas"],//3
    ["US-NV", "Nevada"],//4
    ...
    ["US-AZ", "Arizona"],//6
    ["US-AL", "Alabama"], //7
    ["US-PA", "Pennsylvania"], //8
    ["US-OK", "Oklahoma"], //9
    ["US-MI", "Michigan"]
]

struct Unit4Lesson10: View {
            ...
            VStack{
                if current < 10{
                    ...
                    if stateOfAnswer == 0{
                        ...
                        Text("\(unit4lesson10[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.white)
                    }else if stateOfAnswer == 1{
                        Text("Click on").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                        Text("\(unit4lesson10[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                    }else{
                        Text("Click on").fontWeight(.heavy).font(.largeTitle).foregroundColor(.red)
                        Text("\(unit4lesson10[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.red)
                    }
                        if correct >= 0.7{
                    ...
                    Button {
                        ...
                        if isTapped == false{
                            isTapped = true
                            if clickedPath.name == unit4lesson10[current][0]{
                                stateOfAnswer = 1
                                correct = correct + 0.1
                                pathsCorrect.append(PathData(name: "\(unit4lesson10[current][0])", id: "\(unit4lesson10[current][0])", path: []))
                            }else{
                                stateOfAnswer = 2
                                GlobalUserData.hearts -= 1
                                pathsWrong.append(PathData(name: "\(unit4lesson10[current][0])", id: "\(unit4lesson10[current][0])", path: []))
                            }
...
struct Unit4Lesson10_Previews: PreviewProvider {
    static var previews: some View {
        Unit4Lesson10(GlobalUserData: userData())
    }
}
