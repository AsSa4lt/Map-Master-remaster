...
import SwiftUI
public let unit4lesson11: [[String]] = [
    ["US-KS", "Kansas"],//1
    ["US-KY", "Kentucky"],//2
    ["US-MS", "Mississippi"],//3
    ["US-NM", "New Mexico"],//4
    ["US-VA", "Virginia"],//5
    ["US-WV", "West Virginia"],//6
    ["US-SD", "South Dakota"], //7
    ["US-MN", "Minnesota"], //8
    ["US-OR", "Oregon"], //9
    ["US-MT", "Montana"]
]

struct Unit4Lesson11: View {
            ...
            VStack{
                if current < 10{
                    ...
                    if stateOfAnswer == 0{
                        ...
                        Text("\(unit4lesson11[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.white)
                    }else if stateOfAnswer == 1{
                        Text("Click on").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                        Text("\(unit4lesson11[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                    }else{
                    ...
                    Button {
                        ...
                        if isTapped == false{
                            isTapped = true
                            if clickedPath.name == unit4lesson11[current][0]{
                                stateOfAnswer = 1
                                correct = correct + 0.1
                                pathsCorrect.append(PathData(name: "\(unit4lesson11[current][0])", id: "\(unit4lesson11[current][0])", path: []))
                            }else{
                                stateOfAnswer = 2
...
struct Unit4Lesson11_Previews: PreviewProvider {
    static var previews: some View {
        Unit4Lesson11(GlobalUserData: userData())
    }
}
