...
import SwiftUI
public let unit4lesson12: [[String]] = [
    ["US-IA", "Iowa"],//1
    ["US-HI", "Hawaii"],//2
    ["US-FL", "Florida"],//3
    ["US-WI", "Wisconsin"],//4
    ["US-WY", "Wyoming"],//5
    ["US-UT", "Utah"],//6
    ["US-NC", "North Carolina"], //7
    ["US-ND", "North Dakota"], //8
    ["US-MO", "Missouri"], //9
    ["US-LA", "Louisiana"]
]

struct Unit4Lesson12: View {
            ...
            VStack{
                if current < 10{
                    ...
                    if stateOfAnswer == 0{
                        ...
                        Text("\(unit4lesson12[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.white)
                    }else if stateOfAnswer == 1{
                        Text("Click on").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                        Text("\(unit4lesson12[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                    }else{
                        Text("Click on").fontWeight(.heavy).font(.largeTitle).foregroundColor(.red)
                        Text("\(unit4lesson12[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.red)
                    }
                        if correct >= 0.7{
                    ...
                    Button {
                        ...
                        if isTapped == false{
                            isTapped = true
                            if clickedPath.name == unit4lesson12[current][0]{
                                stateOfAnswer = 1
                                correct = correct + 0.1
                                pathsCorrect.append(PathData(name: "\(unit4lesson12[current][0])", id: "\(unit4lesson12[current][0])", path: []))
                            }else{
                                stateOfAnswer = 2
                                GlobalUserData.hearts -= 1
                                pathsWrong.append(PathData(name: "\(unit4lesson12[current][0])", id: "\(unit4lesson12[current][0])", path: []))
                            }
...
struct Unit4Lesson12_Previews: PreviewProvider {
    static var previews: some View {
        Unit4Lesson12(GlobalUserData: userData())
    }
}
