...
import SwiftUI
public let unit4lesson4: [[String]] = [
    ["DE", "Germany"],//1
    ["US", "United States"],//2
    ["GB", "United Kingdom"],//3
    ["RU", "Russia"],//4
            ...
            VStack{
                if current < 10{
                    ...
                    if stateOfAnswer == 0{
                        ...
                        Text("\(unit4lesson4[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.white)
                    }else if stateOfAnswer == 1{
                        Text("Click on").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                        Text("\(unit4lesson4[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.green)
                    }else{
                        Text("Click on").fontWeight(.heavy).font(.largeTitle).foregroundColor(.red)
                        Text("\(unit4lesson4[current][1])").fontWeight(.heavy).font(.largeTitle).foregroundColor(.red)
                    }
                        if correct >= 0.7{
                    ...
                    Button {
                        ...
                        if isTapped == false{
                            isTapped = true
                            if clickedPath.name == unit4lesson4[current][0]{
                                stateOfAnswer = 1
                                correct = correct + 0.1
                                pathsCorrect.append(PathData(name: "\( unit4lesson4[current][0])", id: "\( unit4lesson4[current][0])", path: []))
                            }else{
                                stateOfAnswer = 2
                                GlobalUserData.hearts -= 1
                                pathsWrong.append(PathData(name: "\( unit4lesson4[current][0])", id: "\( unit4lesson4[current][0])", path: []))
                            }
...
struct Unit4Lesson4_Previews: PreviewProvider {
    static var previews: some View {
        Unit4Lesson4(GlobalUserData: userData())
    }
}
